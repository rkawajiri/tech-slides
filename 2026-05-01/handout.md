...
**含意5: 検証困難なドメインこそ深いモートを築ける**
含意4が synthesizability(合成で済むか)の軸だったのに対し、これは verifiability(機械的に正解判定できるか)の軸の話。verifiable な領域は RLVR + teacher-student合成でコモディティ化する一方、**評価基準が言語化しにくい / 人間の感覚への依存が強い / 評価基準そのものを時間をかけて積み上げる必要がある / 真の正解はユーザーから集めないと得られない**領域は、参入障壁が原理的に高い。

- **言語化困難な品質**: 法律ドキュメントの「読みやすさ」、医療カルテの「診療を反映しているか」、UXコピーの「ブランドらしさ」、教育コンテンツの「学習効果」
- **人間感覚依存**: クリエイティブ、物語、ロールプレイ、コンパニオン、カウンセリング、デザイン
- **評価基準の積み上げ**: 法律事務所ごとのドラフティング規約、企業ごとの社内文体、医療機関の記述規範
- **真のラベルが現場でしか取れない**: 顧客サポートの解決満足度、営業会話の成約寄与、エンジニアリング判断の長期的成否

これらは RLVR が直接効かないため、汎用LLM + 合成では原理的に追いつけない。代わりに必要なのは「**長期間ユーザーと付き合いながら、人間専門家のフィードバックを構造化し、独自の評価基準を時間をかけて育てていく能力**」 — Harvey、Glean、専門特化エージェントが取っているポジション。

つまり段階3の本質は、「verifiable な箱でデータを集める」ことではなく、「**verifiable で*ない*領域に、verifiable な評価インフラを後から建てる**」こと、と読み直せる。bias-corrected judge / IRT-based rubric / ドメイン専門家ラベル — これら全てが「非verifiable領域を段階5まで持って行くインフラ」と読める。
...
ただし**この性質は両刃の剣**で、verifiable な領域ほど他社も RLVR + teacher-student合成で短期間で追いつける、すなわち**コモディティ化のスピードも速い**(Open-Reasoner-Zeroが DeepSeek-R1-Zero を1/10ステップで再現したのが象徴的)。Cursor、Midjourney、Perplexityの現時点の優位は、ドメイン構造そのものの参入障壁よりも、段階1で先行してデータを取り始めた **時間優位** に依る部分が大きい。verifiable性に頼ったモートは賞味期限が短く、(d)「狭く特化したドメイン」や前述の含意5「人間専門家の暗黙知」と組み合わせないと持たない。
...
- [ ] 「**言語化困難な品質**」(ブランドらしさ、読みやすさ、ドメイン固有の規範)を捕捉する仕組みがあるか? 明示的なpreference signalだけに頼っていないか?
- [ ] 評価基準そのものが**時間とともに精緻化**されていく設計か?(初期rubricで固定でなく、ユーザー対話と専門家ラベルからrubric自体が更新される)
...
- [ ] ドメインが **verifiable**(コンパイル可能、引用検証可能、人間選択など機械的検証が可能) か、**verification-hard**(言語化困難、人間感覚依存、評価基準の積み上げが必要) かを意識して設計しているか?
  - **verifiable** の場合: 競合が RLVR + teacher-student合成で追いつく前に、時間優位を確立できているか? Big Techが同等機能を出した瞬間に蒸発するのが verifiable 領域の典型的な死に方
  - **verification-hard** の場合: 人間専門家のフィードバックを構造化し、評価基準(rubric)を時間をかけて育てる仕組みを持っているか? bias-corrected judge / IRT rubric / domain expertラベルへの投資計画があるか?
